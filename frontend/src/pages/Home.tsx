          ...
          <div className="hero-visual" data-testid="hero-visual">
            ...
            <div className="hero-flow-card" data-testid="hero-energy-flow">
              ...
              <div className="hero-flow-list">
                {[{ label: "Factory", icon: Factory }, { label: "Smart Meter", icon: Gauge }, { label: "Analytics", icon: BarChart3 }, { label: "Verified Savings", icon: ShieldCheck }, { label: "UEC", icon: CircleDollarSign }, { label: "Green Wallet", icon: Wallet }].map(({ label, icon: FlowIcon }, index) => (
                  <div className="hero-flow-node" key={label} data-testid={`hero-flow-node-${index}`}><span className="flow-node-icon"><FlowIcon size={14} /></span><span>{label}</span>{index < 5 && <ArrowRight size={12} className="flow-arrow" />}</div>
                ))}
              </div>
            </div>
